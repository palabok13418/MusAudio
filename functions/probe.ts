export async function onRequest(context: any): Promise<Response> {
  ...
  const base = String(env.PROBE_BACKEND_URL || env.DECODE_BACKEND_URL || env.DEMUCS_BACKEND_URL || '').trim();
  if (!base) {
    return new Response(
      JSON.stringify({ ok: false, error: 'missing_probe_backend', hint: 'Set PROBE_BACKEND_URL, DECODE_BACKEND_URL or DEMUCS_BACKEND_URL' }),
